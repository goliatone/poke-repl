...
const DEFAULTS = {
    ...
    logger: console,
    encrypt: encrypt,
    //NOTE: When we use extend, we end up overwritting the properties 
    //of these objects :/
    // stdin: process.stdin,
    // stdout: process.stdout,
    makeStdin: () => process.stdin,
    makeStdout: () => process.stdout,
...
class Client {
    ...
    init(config) {
         ...
         */
        this._handshackeHanler = this.handshacke.bind(this);

        this.socket.on('data', this._handshackeHanler);
    ...
    handshacke(data) {
        ...
        switch (command.label) {
            ...
            case Commands.AUTHORIZE:
                ...
                assert.isString(this.user, () => errored = ERRORS.MISSING_USER);
                assert.isString(this.pass, () => errored = ERRORS.MISSING_PASS);
            ...
            case Commands.CONNECT:
                this.socket.removeListener('data', this._handshackeHanler);
    ...
    handleConnection() {
        ...
        this.stdin.on('end', () => this.socket.destroy());
