...
const ERRORS = {
    ...
    CONNECTION_TIMEOUT: 'poke-repl connection timed out.'
};
...
let TLS_OPTIONS = {
    // key: fs.readFileSync('./tls/poke-certs/client.poke.io.key'),
    // cert: fs.readFileSync('./tls/poke-certs/client.poke.io.crt'),
    // /*
    //  * Needed since the client uses self-signed certs
    //  * and we care about implicit auth.
    //  */
    // ca: [
    //     fs.readFileSync('./tls/poke-certs/poke-server.crt')
    // ],
    ...
    checkServerIdentity: function(host, cert) {
        // console.log('Check server id', host, cert);
...
module.exports = function $client(config){
    ...
    let timeoutId = setTimeout(()=>{
        closeConnection(ERRORS.CONNECTION_TIMEOUT);
